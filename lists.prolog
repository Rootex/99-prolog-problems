...
%some of the below codes are found in opensource communities some written by me
...
%generating the list of all prime numbers whithin a range
prime_list(A,B,L) :- 
    A1 is (A // 2) * 2 + 1,
    p_list(A1,B,L).
...
%Number of primes between 0 and a number X
prime(X,N):- prime_list(0, X, L), length(L, N).        
